...
export type FactFieldState =
  "verified" | "quarantined" | "needs_review" | "last_known_good" | "stale";
export type ValidTimeSource =
  "explicit" | "publication_time" | "observation_time" | "unknown";
...
function factKey(
  fact: Pick<FactVersion, "projectId" | "entityId" | "predicate">,
) {
...
function assertProvenance(provenance: FactProvenance) {
  ...
  if (required.some((value) => !value))
    throw new Error("Fact provenance is incomplete.");
  if (
    provenance.sourcePaths.length === 0 ||
    provenance.evidenceRefs.length === 0
  ) {
    throw new Error(
      "Every fact requires source paths and evidence references.",
    );
  }
}

function releasedVersionIdsAt(
  ...
  const released = new Set(
    history.versions
      .filter(
        (version) =>
          version.transactionFrom <= transactionAt &&
...
export function appendFactVersion(
  ...
  if (
    history.versions.some(
      (version) => version.operationKey === input.operationKey,
    )
  ) {
    ...
    return {
      history,
      version: existing,
      transition: history.transitions.find(
        (item) => item.nextFactVersionId === existing.id,
      ),
      duplicate: true,
    };
  }
  ...
  const canRelease =
    input.fieldState === "verified" || input.fieldState === "last_known_good";
  ...
  const priorVersions = history.versions.filter(
    (item) => factKey(item) === factKey(version),
  );
  if (
    input.intent === "initial" &&
    priorVersions.some((item) => item.state === "released")
  ) {
    throw new Error(
      "Initial fact already exists for this entity and predicate.",
    );
  }
  if (input.intent !== "initial") {
    if (!input.previousFactVersionId)
      throw new Error(`${input.intent} requires a previous fact version.`);
    const previous = history.versions.find(
      (item) => item.id === input.previousFactVersionId,
    );
    if (!previous || factKey(previous) !== factKey(version))
      throw new Error(
        "Previous fact version is missing or belongs to another fact.",
      );
  ...
  if (input.intent !== "initial" && state === "released") {
    transition = {
      ...
      externalEffectiveAt:
        input.intent === "real_change" ? input.validFrom : undefined,
      reason: input.reason,
    };
  }
  return {
...
export function retractFactVersion(
  ...
  const existing = history.transitions.find(
...
function selectedVersionAtTransaction(
  ...
  return history.versions
    .filter(
      (version) => factKey(version) === key && releasedIds.has(version.id),
    )
...
export function regenerateCurrentFacts(
  ...
  return keys.flatMap((key) => {
    ...
    const policy = policies.find(
      (item) => item.predicate === version.predicate,
    ) ?? {
...
export function queryValidAt(
  ...
  const releasedIds = releasedVersionIdsAt(
    history,
    input.believedAtTransaction,
  );
  const candidates = history.versions
    ...
    .sort((left, right) => {
      const validDifference =
        (right.validFrom ?? right.verifiedAt) -
        (left.validFrom ?? left.verifiedAt);
...
export function assertCurrentFactsRegenerate(
  ...
  const comparable = (facts: CurrentFact[]) =>
    facts
      .map(({ regeneratedAt: _regeneratedAt, ...fact }) => fact)
      .sort((left, right) =>
        `${left.entityId}:${left.predicate}`.localeCompare(
          `${right.entityId}:${right.predicate}`,
        ),
      );
  return {
