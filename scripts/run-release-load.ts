...
async function main() {
    ...
    samples.push(
      ...(await Promise.all(
        Array.from(
          ...
          (_, index) => fetchSample(offset + index),
        ),
      )),
    );
  }
  ...
  const report = {
    ...
    concurrency,
    successful_requests: samples.filter(
    ...
    transferred_bytes: samples.reduce(
      (total, sample) => total + sample.bytes,
      0,
    ),
    samples,
  };
  if (report.successful_requests !== requestCount)
    throw new Error(
      `Load check failed: ${report.successful_requests}/${requestCount} requests succeeded`,
    );
  await mkdir("benchmarks/results", { recursive: true });
  await writeFile(
    "benchmarks/results/v1.0.1-load.json",
    `${JSON.stringify(report, null, 2)}\n`,
  );
  console.log(
    `Load proof: ${requestCount}/${requestCount} successful, p95 ${report.latency_ms.p95} ms.`,
  );
}
