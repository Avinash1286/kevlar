...
export type Phase11ProofData = {
  proof: Doc & {
    unauthorizedApprovalDenied: boolean;
    crossTenantReadDenied: boolean;
  };
  routerIngestion: Doc & {
    status: string;
    rawPageAccepted: boolean;
    evidenceRefs: string[];
    approvedByUserId?: string;
  };
  promptInjectionIngestion: Doc & {
    status: string;
    promptInjectionBlocked: boolean;
  };
  chaosRun: Doc & { status: string };
  chaosCases: Array<
    Doc & { kind: string; outcome: string; contained: boolean }
  >;
  alerts: Array<
    Doc & { kind: string; severity: string; status: string; runbookKey: string }
  >;
  cost:
    | null
    | (Doc & {
