...
export const loadPhase9Proof = () =>
  safe((convex) => convex.query(proofQuery, {}));
export const loadEvidenceGraph = (rootNodeId?: string) =>
  safe(async (convex) => {
    const resolved =
      rootNodeId ?? (await convex.query(proofQuery, {}))?.rootNodeId;
    return convex.query(evidenceGraphQuery, {
      ...(resolved ? { rootNodeId: String(resolved) } : {}),
      limit: 100,
    });
  });
export const loadEvidenceBundle = (bundleId?: string) =>
  safe(async (convex) => {
    const resolved =
      bundleId ?? (await convex.query(proofQuery, {}))?.evidenceBundleId;
    if (!resolved) return null;
    return convex.query(bundleQuery, { bundleId: String(resolved) });
  });
export const loadBlastRadius = (incidentId?: string) =>
  safe(async (convex) => {
    const resolved =
      incidentId ?? (await convex.query(proofQuery, {}))?.incidentId;
    if (!resolved) return null;
    return convex.query(blastQuery, { incidentId: String(resolved) });
  });
export const loadFleetProof = () =>
  safe((convex) => convex.query(fleetQuery, { limit: 50 }));
