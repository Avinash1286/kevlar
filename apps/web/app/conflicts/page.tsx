  ...
  return (
    <main className="console-shell cdc-console">
      ...
      <section className="gauntlet-heading">
        <div>
          <p>RECONCILIATION · SOURCE CONFLICT COURTROOM</p>
          <h1>Disagreement is evidence, not a coin toss.</h1>
        </div>
        <span className={`state-pill ${open > 0 ? "stale" : "verified"}`}>
          {open} OPEN
        </span>
      </section>
        ...
        {conflicts.map((conflict) => (
          <article key={conflict._id}>
            <header>
              ...
              <time>{new Date(conflict.openedAt).toISOString()}</time>
            </header>
            ...
            <dl>
              <div>
                <dt>Entity</dt>
                <dd>{conflict.entityId}</dd>
              </div>
              <div>
                <dt>Verified candidates</dt>
                <dd>{conflict.candidateObservationIds.length}</dd>
              </div>
              <div>
                <dt>Released fact</dt>
                <dd>{conflict.releasedFactVersionId ?? "last-known-good"}</dd>
              </div>
            </dl>
