  ...
  return (
    <main className="console-shell cdc-console">
      ...
      <section className="gauntlet-heading">
        <div>
          <p>CONFLICT COURTROOM · PREDICATE-SPECIFIC AUTHORITY</p>
          <h1>{conflict?.predicate ?? "Conflict unavailable"}</h1>
          <code>{conflict?._id ?? id}</code>
        </div>
        <span
          className={`state-pill ${conflict?.status === "resolved" ? "verified" : "stale"}`}
        >
      ...
      <section className="courtroom-summary">
        ...
        <article>
          <span>DECISION</span>
          <strong>{decision?.outcome ?? "continue_last_known_good"}</strong>
        ...
        <article>
          ...
          <strong>{conflict?.candidateObservationIds.length ?? 0}</strong>
          <code>
            {conflict?.releasedFactVersionId ?? "last-known-good retained"}
          </code>
        </article>
      </section>
      <section className="candidate-courtroom">
        {(data?.observations ?? []).map((observation) => {
          const fields = (data?.fields ?? []).filter(
            (field) => field.observationId === observation._id,
          );
          return (
            <article key={observation._id}>
              <header>
                <span>{observation.trustState ?? "verified"}</span>
                <time>
      ...
      <footer className="integrity-footer">
        <span>UNRESOLVED DISAGREEMENT WITHHOLDS THE NEW VALUE</span>
        <code>
          {conflict?.releasedFactVersionId ?? "serving last-known-good"}
        </code>
      </footer>
    </main>
  );
