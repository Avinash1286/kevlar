  ...
  return (
    <main className="console-shell cdc-console">
      ...
      <section className="gauntlet-heading">
        <div>
          <p>SEMANTIC CDC · VERIFIED EVENT STREAM</p>
          <h1>Meaningful changes, not noisy page diffs.</h1>
        </div>
        <span className={`state-pill ${proof ? "verified" : "stale"}`}>
          {proof ? "PROVEN" : "OFFLINE"}
        </span>
      </section>
      <section className="metric-grid">
        <article>
          <span>EVENTS</span>
          <strong>{stream.length}</strong>
        </article>
        <article>
      ...
      <section className="cdc-proof-strip">
        <div>
          <span>LAYOUT-ONLY BUSINESS EVENTS</span>
          <strong>{proof?.layoutBusinessEventCount ?? 0}</strong>
        </div>
        <div>
          <span>VERIFIED PRICE EVENTS</span>
          <strong>{proof?.verifiedPriceEventCount ?? 0}</strong>
        </div>
        <div>
          <span>QUARANTINED EVENTS</span>
          <strong>{proof?.quarantinedEventCount ?? 0}</strong>
        </div>
        <div>
          <span>STABLE RETRY IDS</span>
          <strong>
            {proof ? (proof.stableEventIds === false ? "NO" : "YES") : "—"}
          </strong>
        </div>
      </section>
    </main>
