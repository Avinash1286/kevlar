...
if (!rawBody || !signature || !secret) throw new Error("Set the signed Kevlar webhook inputs.");

const { proposal, duplicate } = consumer.consumeSignedWebhook({ rawBody, signature, secret });
if (!duplicate) {
  console.log(`Review required before ${proposal.action}:`, proposal.evidenceRefs);
}
