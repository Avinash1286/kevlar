...
const processBatchRef = makeFunctionReference<"mutation">(
  "phase12Deletion:processBatch",
);
...
export const request = mutation({
  ...
  returns: v.object({
    tombstone: schema.doc("projectDeletionTombstones"),
    duplicate: v.boolean(),
  }),
  handler: async (ctx, args) => {
    ...
    if (duplicate) {
      if (duplicate.projectId !== args.projectId)
        throw new Error("operationKey belongs to another deletion request");
    ...
    if (
      ...
      throw new Error(
        "A verified same-project backup/restore manifest is required before deletion",
      );
    const now = Date.now();
    ...
    return {
      tombstone: (await ctx.db.get("projectDeletionTombstones", tombstoneId))!,
      duplicate: false,
    };
  },
});
...
export const processBatch = internalMutation({
  ...
  handler: async (ctx, args) => {
    const tombstone = await ctx.db.get(
      "projectDeletionTombstones",
      args.tombstoneId,
    );
    if (!tombstone) throw new Error("Deletion tombstone not found");
    if (tombstone.state === "deleted")
      return { state: tombstone.state, stage: tombstone.stage, processed: 0 };
    const now = Date.now();
    const schedule = async () => {
      await ctx.scheduler.runAfter(0, processBatchRef, {
    ...
    if (tombstone.stage === "evidence") {
      const rows = await ctx.db
        .query("evidence")
        .withIndex("by_projectId_and_deletedAt_and_capturedAt", (q) =>
          q.eq("projectId", tombstone.projectId).eq("deletedAt", undefined),
        )
        .take(25);
      for (const evidence of rows) {
        ...
        await ctx.db.patch("evidence", evidence._id, {
          sourceUrl: "[DELETED_WITH_PROJECT]",
          storageId: undefined,
          metadata: { deleted: true, tombstoneId: String(tombstone._id) },
          deletedAt: now,
        });
      }
      await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
        state: "deleting",
        stage: rows.length === 25 ? "evidence" : "run_rows",
        evidenceRedacted: tombstone.evidenceRedacted + rows.length,
        updatedAt: now,
      });
      await schedule();
      return {
    ...
    if (tombstone.stage === "run_rows") {
      const run = await ctx.db
        .query("runs")
        .withIndex(
          "by_projectId_and_payloadDeletionCompletedAt_and_startedAt",
          (q) =>
            q
              .eq("projectId", tombstone.projectId)
              .eq("payloadDeletionCompletedAt", undefined),
        )
        .first();
      if (!run) {
        await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
          stage: "observation_fields",
          updatedAt: now,
        });
        await schedule();
      ...
      const rows = await ctx.db
        .query("rows")
        .withIndex("by_runId_and_deletedAt", (q) =>
          q.eq("runId", run._id).eq("deletedAt", undefined),
        )
        .take(25);
      ...
      if (rows.length < 25)
        await ctx.db.patch("runs", run._id, {
          payloadDeletionCompletedAt: now,
        });
      await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
        rowsRedacted: tombstone.rowsRedacted + rows.length,
        updatedAt: now,
      });
      await schedule();
    ...
    if (tombstone.stage === "observation_fields") {
      const fields = await ctx.db
        ...
        .withIndex("by_projectId_and_deletedAt_and_createdAt", (q) =>
          q.eq("projectId", tombstone.projectId).eq("deletedAt", undefined),
        )
        .take(25);
      for (const field of fields)
        await ctx.db.patch("canonicalObservationFields", field._id, {
          rawValue: { deleted: true },
          normalizedValue: { deleted: true },
          sourcePaths: [],
          evidenceRefs: [],
          transform: { ...field.transform, input: { deleted: true } },
          deletedAt: now,
        });
      await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
        stage:
          fields.length === 25 ? "observation_fields" : "delivery_payloads",
        observationFieldsRedacted:
          tombstone.observationFieldsRedacted + fields.length,
        updatedAt: now,
      });
      await schedule();
      return {
    ...
    if (tombstone.stage === "delivery_payloads") {
      const deliveries = await ctx.db
        ...
        .withIndex("by_projectId_and_payloadDeletedAt_and_createdAt", (q) =>
          q
            .eq("projectId", tombstone.projectId)
            .eq("payloadDeletedAt", undefined),
        )
        .take(25);
      ...
      await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
        stage: deliveries.length === 25 ? "delivery_payloads" : "credentials",
        deliveriesRedacted: tombstone.deliveriesRedacted + deliveries.length,
        updatedAt: now,
      });
      await schedule();
      return {
    ...
    if (tombstone.stage === "credentials") {
      ...
      for (const key of keys)
        await ctx.db.patch("apiKeys", key._id, {
          prefix: "deleted",
          secretHash: `deleted:${String(key._id)}`,
          status: "revoked",
          revokedAt: now,
          deletedAt: now,
        });
      if (keys.length > 0) {
        await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
          credentialsRevoked: tombstone.credentialsRevoked + keys.length,
          updatedAt: now,
        });
        await schedule();
        return {
      ...
      if (endpoint) {
        const secrets = await ctx.db
          ...
          .withIndex("by_endpointId_and_deletedAt_and_version", (q) =>
            q.eq("endpointId", endpoint._id).eq("deletedAt", undefined),
          )
          .take(25);
        for (const secret of secrets)
          await ctx.db.patch("webhookSecretVersions", secret._id, {
            secretHash: `deleted:${String(secret._id)}`,
            secretRef: "[DELETED_WITH_PROJECT]",
            status: "retired",
            retiredAt: now,
            deletedAt: now,
          });
        if (secrets.length < 25)
          await ctx.db.patch("webhookEndpoints", endpoint._id, {
            ...
            status: "disabled",
            activeSecretVersionId: undefined,
            deletedAt: now,
            updatedAt: now,
          });
        await ctx.db.patch("projectDeletionTombstones", tombstone._id, {
          credentialsRevoked: tombstone.credentialsRevoked + secrets.length,
          updatedAt: now,
        });
        await schedule();
        return {
    ...
    const digest = stableHash({
      ...
      evidenceRedacted: tombstone.evidenceRedacted,
      rowsRedacted: tombstone.rowsRedacted,
      observationFieldsRedacted: tombstone.observationFieldsRedacted,
      deliveriesRedacted: tombstone.deliveriesRedacted,
      credentialsRevoked: tombstone.credentialsRevoked,
      completedAt: now,
    });
    await ctx.db.patch("projects", tombstone.projectId, {
      status: "paused",
    ...
    await ctx.db.insert("securityAuditEvents", {
      ...
      decision: "allowed",
      reason:
        "Privacy-bearing payloads deleted; immutable hashes and audit retained",
      redactedPayload: {
        digest,
        evidenceRedacted: tombstone.evidenceRedacted,
        rowsRedacted: tombstone.rowsRedacted,
