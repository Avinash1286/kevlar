...
async function tenantFixture() {
  ...
  const providerUsers = await t.run(async (ctx) => ({
    primary: await ctx.db.insert("users", {
      name: "Primary",
  ...
  const primaryIdentity = {
    subject: String(providerUsers.primary),
    issuer: "https://auth.test",
    tokenIdentifier: `https://auth.test|${providerUsers.primary}`,
    email: "primary@example.test",
  };
  ...
  const primaryUser = await primary.mutation(
...
describe("Phase 11 Convex authorization", () => {
  it("allows a member to read only their own tenant overview", async () => {
    const { primary, primaryOrganizationId, foreignOrganizationId } =
      await tenantFixture();
    await expect(
      primary.query(api.phase11Access.tenantOverview, {
        organizationId: primaryOrganizationId,
      }),
    ).resolves.toMatchObject({ projects: [{ slug: "primary-project" }] });
    await expect(
      primary.query(api.phase11Access.tenantOverview, {
        organizationId: foreignOrganizationId,
      }),
  ...
  it("blocks a foreign organization owner from creating a key in another tenant", async () => {
    ...
    await expect(
      foreign.action(api.phase10Admin.createApiKey, {
        projectId,
        name: "cross-tenant key",
        rawKey: "fixture-key-material-at-least-24",
        scopes: ["facts:read"],
        rateLimitPerMinute: 10,
        operationKey: "phase11:test:foreign-key",
      }),
    ).rejects.toThrow(/Forbidden|tenant|organization/i);
  });

  it("fails closed when project metrics claim a different organization", async () => {
    const {
      t,
      primary,
      projectId,
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("operationsCostSnapshots", {
        ...
        aiCalls: 0,
        estimatedUsd: 0,
        operationKey: "test:cross-tenant-cost",
        capturedAt: 1_787_400_001,
      });
    });
