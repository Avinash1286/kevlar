...
const benchmark = JSON.parse(readFileSync("benchmarks/results/v1.0.0.json", "utf8"));
const e2e = JSON.parse(readFileSync("benchmarks/results/v1.0.0-e2e.json", "utf8"));
const load = JSON.parse(readFileSync("benchmarks/results/v1.0.0-load.json", "utf8"));

describe("Phase 12 release evidence", () => {
  it("contains four measured baselines and no benchmark placeholders", () => {
    expect(benchmark.baselines.map((item: { name: string }) => item.name)).toEqual(["schema_only", "contract_only", "core_kevlar", "full_platform"]);
    expect(JSON.stringify(benchmark)).not.toMatch(/\b(?:todo|tbd|placeholder|coming soon)\b/i);
  });
  ...
  it("binds live E2E and load assertions into committed raw output", () => {
    ...
    expect(e2e.pages.every((page: { status: number }) => page.status === 200)).toBe(true);
